...
class Paint:
    ...
    def __fit(self, vec):
        ...
        # cam = [self.__size[0]//2, self.__size[1]//2, -800]
        # plane_z = 0
        # dz = vec[2] - cam[2]
        # t = (vec[2] - plane_z)/dz
        # x = (cam[0] - vec[0]) * t + vec[0]
        # y = (cam[1] - vec[1]) * t + vec[1]
        # return 100 * x + self.__size[0]//2, 100 * y + self.__size[1]//2
        return [round(100 * coordinate + frame / 2) for coordinate, frame in zip(vec, self.__size)]
    ...
    def __draw_shape(self, thickness=4):
        for start, end in self.__shape.lines:
            pygame.draw.line(self.__screen, WHITE, self.__fit(start), self.__fit(end), thickness)
